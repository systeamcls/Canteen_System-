...
class RateLimitAuth
{
    ...
    {
    // Skip rate limiting if not an auth route
    if (!$this->isAuthRoute($request)) {
        return $next($request);
        }

        $key = $this->resolveRequestSignature($request);
        $maxAttempts = (int) config('security.rate_limit_auth_attempts', 10);
        $decayMinutes = config('security.rate_limit_auth_decay', 300) / 60; // Convert seconds to minutes
    ...
    {
        // Use IP address, path, and method for rate limiting key
        return sha1(
            $request->ip() . '|' .
            $request->path() . '|' .
            $request->method() . '|' .
