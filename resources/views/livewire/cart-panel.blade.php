<div>
    ...
    <div class="cart-container" x-data="{ open: @entangle('isOpen') }">
        ...
        <button @click="open = true" class="modern-cart-button">
            ...
            <svg width="32" height="32" viewBox="0 0 24 24" fill="#FF6B47" stroke="none">
                ...
                <path d="M16 10a4 4 0 0 1-8 0" stroke="white" stroke-width="1.5" stroke-linecap="round"
                    fill="none" />
            </svg>
        ...
        <!-- Modern Cart Overlay -->
        <div class="modern-cart-overlay" x-show="open" x-transition:enter="transition ease-out duration-300"
            x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100"
            x-transition:leave="transition ease-in duration-200" x-transition:leave-start="opacity-100"
            x-transition:leave-end="opacity-0" @click="open = false" style="display: none;">

            <div class="modern-cart-sidebar" x-transition:enter="transition ease-out duration-300 delay-100"
                x-transition:enter-start="transform translate-x-full" x-transition:enter-end="transform translate-x-0"
                x-transition:leave="transition ease-in duration-200" x-transition:leave-start="transform translate-x-0"
                x-transition:leave-end="transform translate-x-full" @click.stop>
                ...
                <div class="modern-cart-header orange-header">
                    <div class="header-left">
                        <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="white"
                            stroke-width="2">
                            <path
                                d="M3 3h2l.4 2M7 13h10l4-8H5.4m0 0L7 13m0 0l-1.5 7M7 13l1.5 7m0 0h8m-8 0h.01M19 20h.01" />
                        </svg>
                        ...
                        @foreach ($groupedItems as $vendorId => $vendorItems)
                            <div class="vendor-section">
                                <div class="vendor-header">
                                    ...
                                    <span
                                        class="vendor-name">{{ $vendorItems->first()->vendor->name ?? 'Unknown Vendor' }}</span>
                                </div>
                                    ...
                                    <div class="modern-cart-item">
                                        ...
                                        <div class="modern-quantity-controls">
                                            <button
                                                ...
                                                class="qty-btn minus-btn" {{ $item->quantity <= 1 ? 'disabled' : '' }}>
                                        ...
                                        <button wire:click="removeItem({{ $item->id }})" class="remove-item-btn">
                ...
                @if (!empty($cartTotals['items']) && $cartTotals['items']->isNotEmpty())
                    <div class="modern-cart-footer">
                        ...
                        <div class="action-buttons">
                            <button @click="$dispatch('open-delivery-modal')" class="checkout-button">
                                Proceed to Checkout
                            </button>
    ...
    <!-- Delivery Options Modal (INSIDE root wrapper) -->
    @livewire('delivery-options-modal')

</div>
<!-- ✅ END OF SINGLE ROOT WRAPPER -->

@push('scripts')
    <script>
        document.addEventListener('livewire:init', () => {
            Livewire.on('proceedToCheckout', () => {
