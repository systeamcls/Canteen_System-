                        ...
                        <div class="order-summary-card">
                            ...
                            <div class="order-items-list">
                                @php
                                    ...
                                    $showLimit = 3;
                                    $itemsToShow = $showExpandedItems ? $cartSnapshot : array_slice($cartSnapshot, 0, $showLimit);
                                @endphp
                                ...
                                @foreach($itemsToShow as $index => $item)
                                    <div class="order-item" wire:key="item-{{ $index }}">
                                        ...
                                        <div class="item-content">
                                            <div class="item-info">
                                                <h5 class="item-title">{{ $item['product_name'] }} - {{ $item['vendor_name'] }}</h5>
                                                <p class="item-subtitle">Traditional Filipino cuisine with authentic flavors</p>
                                            </div>
                                        ...
                                        <div class="item-actions">
                                            ...
                                            <div class="price-section">
                                                <div class="unit-price">₱{{ number_format($item['unit_price'] / 100, 0) }} each</div>
                                                <div class="item-total">₱{{ number_format($item['line_total'] / 100, 0) }}</div>
                                            </div>
                                        </div>
                                    </div>
                                @endforeach
                                ...
                                @if($totalItems > $showLimit)
                                    <div class="items-toggle">
                                        @if(!$showExpandedItems)
                                            <button type="button" class="see-more-btn" wire:click="toggleExpandedItems">
                                                <span>See {{ $totalItems - $showLimit }} more items</span>
                                                <i class="chevron-down">▼</i>
                                            </button>
                                        @else
                                            <button type="button" class="see-less-btn" wire:click="toggleExpandedItems">
                                                <span>Show less</span>
                        ...
                        <div class="contact-card">
                            ...
                            <div class="contact-form">
                                ...
                                <div class="input-group">
                                    ...
                                    <select 
                                        wire:model.live="notificationPreference" 
        ...
        @if($showPaymentMethods)
            <div class="payment-methods-dropdown">
                @forelse($this->paymentMethodOptions as $method => $details)
                    <div class="payment-option" wire:click="selectPaymentMethod('{{ $method }}')">
                        <div class="payment-option-content">
                            <div class="payment-icon">💳</div>
                            <div class="payment-details">
                                <div class="payment-name">{{ $details['label'] }}</div>
                                <div class="payment-description">{{ $details['description'] }}</div>
                            </div>
                        </div>
                    </div>
                @empty
    ...
    @else
        ...
        <div class="selected-payment-display">
            <div class="selected-payment-info">
                <div class="payment-icon">💳</div>
                <div class="payment-details">
                    <div class="payment-name">{{ $this->paymentMethodOptions[$selectedPaymentMethod]['label'] ?? $selectedPaymentMethod }}</div>
                    <div class="payment-description">{{ $this->paymentMethodOptions[$selectedPaymentMethod]['description'] ?? '' }}</div>
                </div>
            </div>
            <button type="button" class="change-payment-btn" wire:click="changePaymentMethod">
                Change
            </button>
        </div>
    @endif
